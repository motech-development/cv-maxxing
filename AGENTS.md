...
- Do not unit test repo-owned Sandcastle automation under `.sandcastle`; it is non-app orchestration code. Verify it with type-checking, linting, formatting, dry-run/preflight commands, and focused inspection instead of adding testability seams.
...
- Keep repo-level PRD automation in `.sandcastle`, and use one automation-owned draft pull request per PRD.
