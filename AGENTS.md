...
- The planning checkpoint must present the reconnaissance digest, objective, success criteria, impact surface, implementation strategy, non-goals, acceptance criteria, and verification strategy, then start feature grilling with concrete challenge questions and wait for explicit approval to implement.
...
- For every feature request, use the `grill-with-docs` skill when available and grill the request before implementation through an interactive checkpoint: ask one concrete challenge question at a time, provide a recommended answer, check answers against code and project docs when discoverable, wait for the user's answer or explicit acceptance of uncertainty, and do not replace feature grilling with a written summary of assumptions.
