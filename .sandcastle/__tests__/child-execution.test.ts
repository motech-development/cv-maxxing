...
describe('child execution prompt contracts', () => {
  it('documents implementation and review behavior without history rewriting', async () => {
    ...
    expect(implementPrompt).toContain('Update Desktop Visual Snapshots')
    ...
    expect(combinedPrompts).not.toContain('scripts/update-desktop-visual-snapshots')
    expect(combinedPrompts).not.toContain('.sandcastle/update-desktop-visual-snapshots')
    expect(combinedPrompts).not.toContain('--retry')
    expect(combinedPrompts).not.toContain('retry:')
    expect(combinedPrompts).not.toContain('gh pr comment')
    expect(combinedPrompts).not.toContain('gh issue comment')
