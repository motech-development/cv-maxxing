...
test('captures an authenticated job site vacancy through the internal browser session and restores a ready preview in the app shell', async () => {
  const testPaths = await createOriginalCvTestPaths()
  ...
  const electronApp = await launchDesktopApp({
    ...
    CV_MAXXING_VACANCY_BROWSER_SESSION_RESOLVED_URL: 'https://careers.example.com/jobs/123456',
  })
  ...
  await page.getByLabel('Job link').fill('https://careers.example.com/jobs/123456')
...
test('returns cleanly to the vacancy intake with blocking guidance when the internal browser session still exposes an incomplete page', async () => {
  ...
  await expect(page.getByLabel('Job link')).toHaveValue('https://careers.example.com/jobs/123456')
