...
export function createVacancyService({
  ...
  return {
    ...
    ingestPastedVacancy: async ({
      ...
      const source =
        normalizedUrl === null ? 'pasted-job-description' : resolveVacancySource(normalizedUrl)
    ...
    ingestVacancyUrl: async ({ url }: { url: string }): Promise<VacancyIngestResult> => {
      ...
      const source = resolveVacancySource(normalizedUrl)
...
function resolveVacancySource(url: string): VacancySource {
...
function isExpectedBrowserSessionVacancyPage({
  ...
  try {
    ...
    return (
      normalizeHostname(currentUrl.hostname) === normalizeHostname(requestedUrl.hostname) &&
