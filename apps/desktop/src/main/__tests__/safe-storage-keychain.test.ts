...
import { SafeStorageUnavailableError, createSafeStorageKeychain } from '../safe-storage-keychain.js'
...
test('fails closed when safeStorage cannot access OS-backed encryption', async () => {
  ...
  await expect(keychain.getOrCreateAppDataKey()).rejects.toBeInstanceOf(SafeStorageUnavailableError)
})
