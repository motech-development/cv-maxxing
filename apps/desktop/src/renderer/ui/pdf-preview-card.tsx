...
export function PdfPreviewCard({ preview }: PdfPreviewCardProperties) {
  if (preview === null) {
    return (
  ...
  return <LoadedPdfPreviewCard key={preview.id} preview={preview} />
}

function LoadedPdfPreviewCard({ preview }: { preview: TailoredApplicationPreview }) {
  ...
  return (
    <div className="flex h-full min-h-[520px] flex-col gap-4 rounded-[8px] border border-[var(--color-border)] bg-[var(--color-surface-1)] p-[18px]">
      ...
      <div className="flex min-h-0 flex-1 items-start justify-center overflow-auto rounded-[6px] bg-[var(--color-shell-canvas)] p-4">
        ...
        <canvas
          aria-label="Adapted CV PDF preview"
